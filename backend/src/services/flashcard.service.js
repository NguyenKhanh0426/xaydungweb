...
export const flashcardService = {
  async createFlashcard(userId, vocabularyId) {
    const vocabulary = await vocabRepository.findVocabularyByIdForUser(vocabularyId, userId);
    if (!vocabulary) {
      throw new ApiError(404, 'Vocabulary not found');
    }

    const existing = await flashcardRepository.findExistingFlashcard(userId, vocabularyId);
    if (existing) {
      throw new ApiError(409, 'Flashcard for this word already exists');
    }
