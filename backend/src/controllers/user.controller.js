...
export const userController = {
  ...
  getGoals: async (req, res) => {
    const data = await goalService.getGoals(req.user.id, req.query.status || null);
  ...
  updateGoal: async (req, res) => {
    const data = await goalService.updateGoal(req.user.id, Number(req.params.goalId), req.body);
    return ok(res, data, 'Goal updated');
  }
};
